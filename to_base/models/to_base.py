...
class TOBase(models.AbstractModel):
    ...
    def get_partner_tz(self, partner):
        tz = partner.tz
        if not tz and partner.country_id:
            try:
                timezones = pytz.country_timezones(partner.country_id.code)
                if timezones:
                    tz = timezones[0]
            except Exception:
                pass  # Ignore error when country code is not valid (in test data)
        return tz or 'UTC'
    ...
    def get_company_tz(self, company=None):
        ...
        return self.get_partner_tz(company.partner_id)
    ...
    def convert_local_to_utc(self, dt, force_local_tz_name=None, is_dst=None, naive=False):
        ...
        if given_is_date:
            dt = self.date_to_datetime(dt, naive=naive)
    ...
    def convert_utc_to_local(self, utc_dt, force_local_tz_name=None, is_dst=None, naive=False):
        ...
        if given_is_date:
            utc_dt = self.date_to_datetime(utc_dt, naive=naive)
    ...
    def date_to_datetime(self, date, tzinfo=None, naive=True):
        ...
        """
        now_time = self.float_hours_to_time(self.time_to_float_hour(fields.Datetime.now()), tzinfo=tzinfo)
        if naive:
    ...
    def float_hours_to_time(self, float_hours, tzinfo=None):
        ...
        :rtype: time
        """
        total_microseconds = round(float_hours * 60 * 60 * 1000000)
        hours, remainder = divmod(total_microseconds, 60 * 60 * 1000000)
        minutes, remainder = divmod(remainder, 60 * 1000000)
        seconds, microseconds = divmod(remainder, 1000000)
        if isinstance(tzinfo, str):
            tzinfo = pytz.timezone(tzinfo)
        return date_utils.time(hours, minutes, seconds, microseconds, tzinfo)
    ...
    def period_iter(self, period_name, dt_start, dt_end, start_day_offset=0, cycle_value=1):
        ...
        if isinstance(dt_start, datetime) != isinstance(dt_end, datetime):
    ...
    def validate_year(self, year):
        ...
        """
        if year and (isinstance(year, int) or year.isdigit()):
            year_int = int(year)
