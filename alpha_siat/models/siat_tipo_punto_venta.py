# alpha_siat/models/siat_tipo_punto_venta.py
import logging
...
class SiatTipoPuntoVenta(models.Model):
    _name = "alpha.siat.tipo.punto.venta"
    _description = "SIAT - Tipos de Punto de Venta"
    ...
    codigo_clasificador = fields.Integer(
        ...
        required=True,
        index=True,
        help="Código del tipo de punto de venta SIAT"
    )

    descripcion = fields.Char(
        string="Descripción",
        required=True,
        help="Descripción del tipo de punto de venta"
    )
    ...
    def sync_from_siat_response(self, company, tipos_list):
        """
        Synchronize point-of-sale types from SIAT response.
        :param company: res.company
        :param tipos_list: list of dicts {'codigoClasificador': '1', 'descripcion': 'PUNTO ...'}
        :return: dict with stats
        """
        if not tipos_list:
            _logger.warning("No punto venta types to synchronize")
        ...
        for t in tipos_list:
            ...
            try:
                ...
                _logger.warning("Invalid codigoClasificador for tipo punto venta: %s", codigo)
                continue
        ...
        _logger.info("SIAT TipoPuntoVenta sync for %s: %d created, %d updated, %d deactivated",
