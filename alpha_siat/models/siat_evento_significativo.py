...
class SiatEventoSignificativo(models.Model):
    _name = "alpha.siat.evento.significativo"
    _description = "SIAT - Eventos Significativos"
    ...
    codigo_clasificador = fields.Integer(
        ...
        required=True,
        index=True,
        help="Código del evento significativo SIAT"
    )

    descripcion = fields.Text(
        string="Descripción",
        required=True,
        help="Descripción del evento significativo que justifica operación offline"
    )
    ...
    @api.model
    def get_evento_by_codigo(self, codigo):
        """
        Get event description by code

        :param codigo: Event code (int or str)
        :return: Event description string or None
        """
        try:
            codigo = int(codigo)
        ...
        return evento.descripcion if evento else None

    @api.model
    def sync_from_siat_response(self, company, eventos_list):
        """
        Synchronize significant events from SIAT response

        :param company: res.company record
        :param eventos_list: list of dicts with codigoClasificador, descripcion
        :return: dict with statistics
        """
        if not eventos_list:
            _logger.warning("No significant events to synchronize")
        ...
        for evento_data in eventos_list:
            codigo = evento_data.get('codigoClasificador', '')
            ...
            vals = {
                ...
                'codigo_clasificador': codigo,
                'descripcion': evento_data.get('descripcion', '').strip(),
        ...
        _logger.info(
            "SIAT Significant Events sync completed for %s: %d created, %d updated, %d deactivated",
