# alpha_siat/models/siat_unidad_medida.py
import logging
...
class SiatUnidadMedida(models.Model):
    _name = "alpha.siat.unidad.medida"
    _description = "SIAT - Unidades de Medida"
    ...
    codigo_clasificador = fields.Integer(
        ...
        help="Código del tipo de unidad SIAT"
    )
    ...
    # Optional: reference to real Odoo uom
    uom_id = fields.Many2one(
        'uom.uom',
        string='UoM (Odoo)',
        help='Mapea la unidad SIAT con una unidad de medida de Odoo (opcional)'
    )
    ...
    def sync_from_siat_response(self, company, tipos_list):
        """
        Synchronize units of measure from SIAT response.
        tipos_list: [{'codigoClasificador': '1', 'descripcion': 'BOBINAS'}, ...]
        """
        if not tipos_list:
            _logger.warning("No unidad medida to synchronize")
        ...
        for t in tipos_list:
            ...
            try:
                ...
                _logger.warning("Invalid codigoClasificador for unidad medida: %s", codigo)
                continue
        ...
        _logger.info("SIAT UnidadMedida sync for %s: %d created, %d updated, %d deactivated",
    ...
    # Convenience helper examples
    @api.model
    def get_codigo_unidad_bienes(self, company=None):
        """Return likely SIAT code for 'UNIDAD (BIENES)' (example: 57 in sample)."""
        domain = [('descripcion', 'ilike', 'UNIDAD (BIENES)'), ('active', '=', True)]
        if company:
            domain = [('company_id', '=', company.id)] + domain
        rec = self.search(domain, limit=1)
        return rec.codigo_clasificador if rec else 57

    @api.model
    def get_codigo_unidad_servicios(self, company=None):
        """Return likely SIAT code for 'UNIDAD (SERVICIOS)' (example: 58)."""
        domain = [('descripcion', 'ilike', 'UNIDAD (SERVICIOS)'), ('active', '=', True)]
        if company:
            domain = [('company_id', '=', company.id)] + domain
        rec = self.search(domain, limit=1)
        return rec.codigo_clasificador if rec else 58
