...
class Message(models.Model):
    ...
    def create(self, values_list):
        ...
        messages = super(
            Message, self.with_context({"whatsapp_marketing": True, "is_automated_action": self._context.get('is_automated_action', False),"report_taken": self._context.get('report_taken', False)})
        ).create(values_list)
                ...
                if user.id != self.env.ref("base.public_user").id:
                    if values.get("model") == "discuss.channel":
                        ...
                        if channel_company_line_id.partner_id.mobile:
                            channel_company_line_id.partner_id.mobile.strip(
                                "+"
                            ).replace(" ", "")
                        ...
                        if provider_id:
                            ...
                            if "whatsapp_messaging_id" in values:
                                vals.update(
                                    {
                                        "whatsapp_messaging_id": values.get(
                        ...
                        if values.get("model") == "res.partner":
                            ...
                            if data[0]["partner_id"]:
                                ...
                                raise AccessError(
                                    _("Partner must be Required for whatsapp message!")
                                )
                        else:
                            raise AccessError(
                                _("Partner must be Required for whatsapp message!")
                        ...
                        if partner and partner.mobile:
                            ...
                            provider_id = self.env.context.get('provider_id') if self.env.context.get('provider_id') else user.provider_id
                            if provider_id:
                                provider_channel_id = (
                                    partner.channel_provider_line_ids.filtered(
                                        lambda s: s.provider_id == provider_id
                                    )
                                )
                            else:
                            ...
                            if provider_channel_id:
                                ...
                                if (
                                    ...
                                    channel.sudo().write(
                                        {"channel_partner_ids": [(4, user_partner.id)]}
                                    )
                            else:
                                name = partner.mobile
                                ...
                                if (
                                    provider_id
                                    and user.id == self.env.ref("base.public_user").id
                                ):
                                    partner.write(
                                        {
                            ...
                            message_comment = (
                                self.env["mail.message"].sudo().create(message_values)
                            )

                            # phone change to mobile
                            partner.mobile.strip("+").replace(" ", "")
                            if provider_id:
                                # phone change to mobile
                                vals = {
                                    "provider_id": provider_id.id,
                            ...
                            for mes in messages:
                                mes.sudo().write({
                                    ...
                                    'chatter_wa_message_id': message_comment.id,
                                })
                        ...
                        self.env["whatsapp.history"].sudo().with_context(
                            {
                                ...
                                "cron": self.env.context.get("cron"),
                            }
                        ).create(vals)
