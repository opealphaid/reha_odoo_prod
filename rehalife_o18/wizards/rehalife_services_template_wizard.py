...
class RehalifeServicesTemplateWizard(models.TransientModel):
    ...
    # Columnas de la hoja de carga. Son los nombres TECNICOS de los campos: el
    # importador de Odoo los mapea solo, sin depender del idioma de la interfaz.
    #
    # Deliberadamente NO estan las columnas que Odoo resuelve por su cuenta:
    #   uom_po_id  -> compute+store desde uom_id, se autocompleta y se
    #                 recalcula si cambia la unidad
    #   purchase_ok-> lo apaga product.template.create() cuando el producto es
    #                 un servicio reservable
    #   active     -> default True
    #   rehalife_external_id -> lo llena el backend al sincronizar; en una
    #                 carga manual va vacio siempre
    _COLUMNAS = [
    ...
    # Filas de ejemplo de la hoja "Servicios". Los corchetes marcan lo que hay
    # que reemplazar con datos de las hojas de catalogo. Si alguien importa sin
    # borrarlas, los codigos SIAT entre corchetes no resuelven y la fila falla
    # con un error explicito — no entra basura en silencio.
    _EJEMPLOS = [
        [
