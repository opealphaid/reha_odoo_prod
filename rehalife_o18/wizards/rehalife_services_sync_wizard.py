...
class RehalifeServicesSyncWizard(models.TransientModel):
    ...
    def _run_pull(self):
        ...
        if errores:
            ...
            lines.extend('  - %s' % error for error in errores)
            lines.append('')
            lines.append(
                'El resto se sincronizo igual. Estos quedaron con Estado Sync = '
                'Error y el detalle en su campo Error de Sync; el filtro "No '
                'Sincronizados" del menu Servicios los lista.'
            )
            lines.append(
                'Si el error habla de codigos SIAT: el producto esta marcado '
                'como vendible o disponible en POS pero le falta la '
                'homologacion. Completala en la pestaña "Servicio Rehalife" '
                '(codigos SIAT) o en "Homologacion SIAT", y volve a sincronizar.'
            )
